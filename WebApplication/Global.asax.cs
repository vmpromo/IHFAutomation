...
namespace IHF.ApplicationLayer.Web
{
    public class Global : System.Web.HttpApplication
    {
        ...
        protected void Application_Error(object sender, EventArgs e)
        {
            ...
            try
            {
                var capturedException = Server.GetLastError();
                var user = User.Identity != null ? User.Identity.Name : "";

                new LoggerDAO().LogException(capturedException, user, Request.Path);

                Exception exception = capturedException.GetBaseException();
