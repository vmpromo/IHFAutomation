...
//  1.1      10/12/12    J Watt     Intermediate checkin
//  1.2      11/12/12    J Watt     Intermediate checkin
//  1.3      19/12/12    J Watt     No Change
//  1.4      21/12/12    J Watt     No Change
//  1.5      07/01/13    J Watt     Changes after DC testing
//  1.6      17/01/13    J Watt     No Change
//  1.7      17/01/13    J Watt     No Change
//  1.8      17/01/13    J Watt     Dont allow unsaved changes
//  1.9      21/01/13    J Watt     No Change
//  1.10     22/01/13    J Watt     No Change
//  1.11     21/02/13    J Watt     No Change
//  1.12     26/07/13    J Watt     No Change
//  1.13     05/09/13    J Watt     Dont allow clicking out of scan box
//  1.14     05/09/13    J Watt     Dont allow clicking out of scan box
//  1.15     09/05/17    M Cackett  Cross border returns changes.
//  1.16     18/05/17    M Cackett  CBR call proc to process refund in store.
//  1.17     21/06/17    M Cackett  Allow DC users to change status from Refunded Return.
//  1.18     26/09/17    M Cackett  CBR Bug fix for Store id/ till id
//  1.19     11/10/17    M Cackett  Prevent store users from using CBR app to process
//                                  Store Orders.  Also fixed compiler warning message.
//  1.20     13/10/17    M Cackett  Refactored code into a function
//                       A Petrescu
...
{
    public partial class ReturnsCBR : System.Web.UI.Page
    {
#region constants
        ...
        /// <summary>
        /// Tests if store order anddisplays message;
        /// </summary>
        /// <param name="scannstring"></param>
        /// <returns>True if store order; false otherwise</returns>
        private bool DisplayStoreOrderMessage(string scannstring)
        {
            // If we are called from a store, and the returned order is a store order then
            // display error message and don't allow the user to continue.
