...
{
    public partial class OrdersByHour : System.Web.UI.Page
    {
        ...
        public void BindData_itempacked(DateTime stdt, DateTime enddt)
        {
            ...
            DataSet ds_item = itemdao.Get_ordersbyhour(stdt, enddt);
            ...
            for (int j = 0; j < dayarr.Count; j++)
            {
                for (int i = 0; i < source.Rows.Count; i++)
                {
                    ...
                    if (dayarr[j].ToString() == source.Rows[i][0].ToString())
                    {
                        ...
                        if (i > 0)
                        {
                            prevhr = Int16.Parse(source.Rows[i - 1][1].ToString());
                        }
                        
                        if (hr == prevhr && i > 0)
                        {
                            or = or + Int32.Parse(source.Rows[i][2].ToString());
                            dest.Rows[j][hr + 1] = or.ToString();
                        }
                        else
                        {
                            or = Int32.Parse(source.Rows[i][2].ToString());
                            dest.Rows[j][hr + 1] = or.ToString();
                        }
                    }
                }
