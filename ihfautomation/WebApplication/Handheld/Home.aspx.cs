...
namespace IHF.ApplicationLayer.Web.Handheld
{
    public partial class Home : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            MainMenu menu = new MainMenu(input, user, application);
            ...
            for (int j = 0; j < menu.SubMenu.Count; j++)
            {
                menuItem = (HtmlTableCell)this.Master.FindControl("menuItem" + (j + 1));
                menuItem.InnerText = (j + 1) + " - " + menu.SubMenu[j].Caption.ToString();
                menuItem.Visible = true;

                script.Append("\t \t if (barcode.value == " + (j + 1) + "){\n");
                if (menu.SubMenu[j].PageChildInd == "0")
                {
                    script.Append("\t \t \t  window.navigate('" + menu.SubMenu[j].Url + "');\n");
            ...
            for (int i = menu.SubMenu.Count + 1; i <= 6; i++)
            {
